...
{
    // Reason: Tests (and embedders) still need a hook into the PTY lifecycle,
    // so forward the old pager API onto the shared wrapper.
    termios_hook_ = hook;
}
...
int
wrapper_run(const struct wrapper_config *config)
{
    if (!config || config->argc <= 0 || !config->argv)
        return EXIT_FAILURE;

    int argc    = config->argc;
    char **argv = config->argv;
    nav_mode_t desired_mode =
        (config->mode == NAV_MODE_MAN) ? NAV_MODE_MAN : NAV_MODE_OSC8;
    // Reason: Switching the parser is as simple as updating the global mode,
    // so capture it once at the start of the run.
    nav_set_mode(desired_mode);
...
{
    bool refilled = readq_refill(bq);
    if (!refilled) {
        if (readq_available_bytes(bq) == 0)
            return true;
        // Reason: Signal that the child closed its side so any buffered man
        // tokens flush before we stop reading.
        nav_request_drain();
    }
