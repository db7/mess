...
#
# Changelog:
# - v0.1: initial version with optional candidate command support
#
# ------------------------------------------------------------------------------
set -eu
...
Options:
  ...
  --candidate CMD Check CMD before searching PATH
...
ARCHIVE="$WORKDIR/${PROGRAM_NAME}-${VERSION}.tar.gz"
SRCDIR="$WORKDIR/${PROGRAM_NAME}-${VERSION}"
BINARY="$SRCDIR/$PROGRAM_NAME"

if [ -n "$CANDIDATE" ] && binary_has_version "$CANDIDATE" "$VERSION"; then
	printf '%s\n' "$CANDIDATE"
	exit 0
fi
